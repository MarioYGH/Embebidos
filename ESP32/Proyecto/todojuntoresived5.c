...
void recv_cb(const esp_now_recv_info_t *esp_now_info, const uint8_t *data, int data_len) {
    ...
    // Enviar los datos de los sensores por ESP-NOW
    SHT1x_Sample_t Sample = {0}; // Variable para almacenar la muestra del sensor SHT1x
    sprintf((char *)sensor_data, "Temperature: %.2f°C, Humidity: %.2f%%, Luminosity: %.2fV", Sample.TempCelsius, Sample.HumidityPercent, voltage);

    uint8_t *mac_addr = esp_now_info->src_addr;
    ESP_ERROR_CHECK(esp_now_send_data(mac_addr, sensor_data, strlen((char *)sensor_data)));
}
