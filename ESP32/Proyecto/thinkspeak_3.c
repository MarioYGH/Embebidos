...
{
    ESP_LOGI(TAG, "Data received from: " MACSTR ", Data: %.*s", MAC2STR(mac_addr), data_len, data);

    if (data_len >= sizeof(float) * 2) {
        float received_temp, received_hum;
        memcpy(&received_temp, data, sizeof(float));
        memcpy(&received_hum, data + sizeof(float), sizeof(float));
        ...
        // Actualizar las variables globales si es necesario
