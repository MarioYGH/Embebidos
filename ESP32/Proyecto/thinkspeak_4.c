...
void recv_cb(const uint8_t *mac_addr, const uint8_t *data, int data_len)
{
    ...
    // Copiar los datos recibidos en un búfer temporal
    char temp_buffer[data_len + 1];
    strncpy(temp_buffer, (char *)data, data_len);
    temp_buffer[data_len] = '\0'; // Terminar la cadena con un null terminator

    // Intentar extraer temperatura y humedad usando sscanf
