...
void send_data_to_thingspeak(const char* data)
{
    char post_url[256];
    snprintf(post_url, sizeof(post_url), "https://api.thingspeak.com/update?api_key=0LCIV8KQ22AXZNR6&field1=%s&field2=%s&field3=%s", data, data, data);
    ...
    if (err == ESP_OK) {
        ...
        if (status_code == 200) {
            ...
            ESP_LOGI(TAG, "Message sent failed with status code: %d", status_code);
        }
    } else {
...
void recv_cb(const uint8_t *mac_addr, const uint8_t *data, int data_len)
{
    ...
    if (is_wifi_connected()) {
        send_data_to_thingspeak(received_data);
    } else {
        ESP_LOGW(TAG, "Not connected to Wi-Fi, cannot send data to ThingSpeak");
    }
}
