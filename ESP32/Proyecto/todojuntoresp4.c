...
void sht1x_task(void *pvParameter) {
    ...
    while (true) {
        ...
        temperature = Sample.TempCelsius;
        humidity = Sample.HumidityPercent;

        // Preparar los datos para enviarlos por ESP-NOW
        uint8_t data[12]; // Tamaño total de los datos a enviar
        data[0] = 'T'; // Identificador de tipo de dato Temperature
        *(float *)(data + 1) = temperature;
        data[5] = 'H'; // Identificador de tipo de dato Humidity
        *(float *)(data + 6) = humidity;
        data[10] = 'L'; // Identificador de tipo de dato Luminosity
        *(float *)(data + 11) = voltage;

        // Enviar datos por ESP-NOW
        esp_err_t ret = esp_now_send_data(peer_mac, data, sizeof(data));
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Error sending data over ESP-NOW: %s", esp_err_to_name(ret));
        }
