...
esp_err_t mcpwm_config(){
    ...
    // go low on compare threshold
    ESP_ERROR_CHECK(mcpwm_generator_set_action_on_compare_event(generator2, MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP, comparator, MCPWM_GEN_ACTION_LOW)));
...
void mover_servo(void *pvParameters){
    ...
    if(strlen(user) == 0 && strlen(pass)==0 && gpio_get_level(SENSOR_IR)==1 && bandera == 0){
        ...
        angle1=0;
        angle2=60;
        ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(comparator, angle_to_compare1(angle2)));
        vTaskDelay(pdMS_TO_TICKS(1000));
        angle2=0;
